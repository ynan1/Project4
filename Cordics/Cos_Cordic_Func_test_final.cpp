...
int main()
{
	...
	for (int test_angle_fn = -1000000; test_angle_fn < 1000001; test_angle_fn++)
	{
		...
		float cos_v = fn(test_angle_fn_fl);

		float cos_v_real = cos(test_angle_fn_fl);

		float error_abs_cos = abs(cos_v_real - cos_v);  // cos_v_real) * 100;

		if (error_abs_cos > .0003 && abs(cos_v) >= .001 && abs(cos_v) < 1.)  //&& abs(cos_v) >= .1

		{
			printf("Error is more than .0003  \n");
			printf("cos(%f%s) is = %f  %f    %f \n", test_angle_fn_fl, " radian", cos_v, cos_v_real, error_abs_cos);
		}
