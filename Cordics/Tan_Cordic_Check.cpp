...
float tann(float test_angle)
{
	...
	if (angle_radian > 3.14158)
	{
		angle_radian = (angle_radian - 6.28318);
	}
	else if (angle_radian < -3.14158)
	{
		angle_radian = (angle_radian + 6.28318);
	}

	if (angle_radian > 0 && angle_radian < 1.570796)
	{
	...
	else if (angle_radian < 0 && angle_radian > -1.570796)
	{
		angle_temp = angle_radian;
		cos_v = initial_x;
		sin_v = initial_y;
	...
	if (angle_radian == 0. || angle_radian == 6.28318 || angle_radian == -0. || angle_radian == -6.28318) // IF ANGLE IS PLUS MINUS 0 OR 360
	...
	if (angle_radian == 3.14159 || angle_radian == -3.14159) // IF ANGLE IS PLUS MINUS 180 
		// TAKE THE EXACT VALUES OF cos AND sin WITHOUT CALCULATING, THOUGH PROGRAM
	{                   // IS CALCULATING THEM BUT OVERRIDE THOSE CALCULATED VALUES WITH EXACT VALUES
...
int main()
{
	...
	for (int test_angle_fn = -1000000; test_angle_fn < 1000001; test_angle_fn++)
	{
		...
		long double tan_v = tann(test_angle_fn_fl);
		...
		{
			printf("Tan(%f%s) is = %f  %f %f %s\n", test_angle_fn_fl, " radian", tan_v, tan_v_real, error_percent, "percent");
			printf("Error is more than 100 \n");
		}
		//else
		//{
