...
int main()
{
	...
	double ph[14] = { 0.785398, 0.463648, 0.244979, 0.124355, 0.0624188, 0.0312398, 0.0156237, 0.00781234, 0.00390623, 0.00195312, 0.000976562, 0.000488281, 0.000244141 };
	double ph_temp = 0;
	double x_re = 0;
	double y_im = 0;
	int i;
	...
	std::cout << "\n Sin(" << phase << ") = " << y_cal << "\n Percentage Error in Sin is: " << 100 * ((y_cal - sin(phase * PI / 180)) / sin(phase * PI / 180)) << " % "
		<< "\n \n Cos(" << phase << ") = " << x_cal << "\n Percentage Error in Cos is: " << 100 * ((x_cal - cos(phase * PI / 180)) / cos(phase * PI / 180)) << " % "
		<< "\n \n Tan(" << phase << ") = " << (y_cal / x_cal) << "\n Percentage Error in Tan is: " << 100 * (((y_cal / x_cal) - tan(phase * PI / 180)) / tan(phase * PI / 180)) << " % ";
	std::cout << '\n';
